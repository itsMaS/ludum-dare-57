...
namespace FIMSpace.FSpine
{
    public partial class FSpineAnimator
    {
        ...
        {
            Vector3 basePos = GetBaseTransform().position;
            ...
            Handles.DrawWireDisc(basePos, GetBaseTransform().TransformDirection(ModelUpAxis), _gizmosDist);
            Vector3 fwd = basePos + GetBaseTransform().TransformDirection(ModelForwardAxis.normalized) * _gizmosDist;
            ...
            Handles.DrawLine(fwd, Vector3.LerpUnclamped(basePos, fwd, 0.5f) + rght * _gizmosDist * 0.2f);
            Handles.DrawLine(fwd, Vector3.LerpUnclamped(basePos, fwd, 0.5f) - rght * _gizmosDist * 0.2f);
            Handles.DrawLine(basePos, basePos + GetBaseTransform().TransformDirection(ModelUpAxis) * _gizmosDist);
            Handles.color = new Color(1f, 1f, 1f, 0.05f);
            Handles.DrawSolidDisc(basePos, GetBaseTransform().TransformDirection(ModelUpAxis), _gizmosDist);

            if (SpineBones.Count > 1)
            {
                Handles.color = new Color(1f, 1f, 1f, 0.25f);
                Handles.DrawDottedLine(basePos, SpineBones[0].transform.position, 2f);
                Handles.DrawDottedLine(basePos, SpineBones[SpineBones.Count - 1].transform.position, 2f);
            }
        }
        ...
        void Gizmos_DrawBonesChainEditorMode()
        {
            ...
            else
            {
                ...
                for (int i = SpineBones.Count - 2; i >= 0; i--)
                {
                    ...
                    SpineBones[i].ProceduralRotation = target.sqrMagnitude <= 0.01f ? Quaternion.identity : Quaternion.LookRotation(target, GetBaseTransform().up);

                    Vector3 dir = SpineBones[i].transform.position - SpineBones[i + 1].transform.position; Quaternion look = dir.sqrMagnitude <= 0.01f ? Quaternion.identity : Quaternion.LookRotation(dir.normalized, GetBaseTransform().up);
